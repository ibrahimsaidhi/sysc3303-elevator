...
public class FloorFormatReader {
	...
	public Pair<Optional<FloorEvent>, Optional<ElevatorErrorEvent>> next() throws IOException {
		...
		if (line.matches(floorRegex)) {
			...
			return (new Pair<> (Optional.empty(), Optional.of(new ElevatorErrorEvent(Integer.parseInt(parts[1]), parts[2].toLowerCase().contains("door") ? ElevatorError.DoorStuck 
					: ElevatorError.StuckBetweenFloors, millis))));
		} else {
			return null;
		}
	}
